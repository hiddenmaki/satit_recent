    ...
    <div>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 py-1">
                ...
                <li class="breadcrumb-item active" aria-current="page"><a href="master-teacher.php" class="text-decoration-none text-muted">ข้อมูลครู</a></li>
            </ol>
        </nav>
    </div>
    <div>
...
<?php if (isset($_GET['status'])): ?>
    ...
    <?php elseif ($_GET['status'] == 'err_delete_fk'): ?>
        <div class="alert alert-warning alert-dismissible fade show shadow-sm border-0" role="alert">
            <i class="fas fa-exclamation-triangle me-2"></i> ไม่สามารถลบได้: ข้อมูลบุคลากรนี้ถูกผูกเข้ากับข้อมููลอื่นในระบบ
            (เช่น ตารางสอน)
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    <?php else: ?>
    ...
    <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center border-bottom">
        ...
        <form method="GET" action="master-teacher.php" class="input-group" style="width: 300px;">
            <input type="text" name="search" class="form-control form-control-sm bg-light border-0"
                placeholder="ค้นหาชื่อ, รหัส..." value="<?= htmlspecialchars($_GET['search'] ?? '') ?>">
        ...
        <div class="table-responsive">
            <table class="table table-custom table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    if (!empty($search)) {
                        $searchParam = $search . '%';
                        $whereClause = " WHERE t.teacher_code LIKE :search1 OR u.first_name LIKE :search2 OR u.last_name LIKE :search3 OR t.line_id LIKE :search4 OR d.name LIKE :search5";
                        $params = [
                            ':search1' => $searchParam,
                            ':search2' => $searchParam,
                            ':search3' => $searchParam,
                    ...
                    $countSql = "SELECT COUNT(*) as total FROM teachers t 
                    ...
                    $sql = "SELECT t.*, d.name AS dept_name, u.first_name, u.last_name, u.prefix, u.username, u.status 
                            ...
                            LEFT JOIN departments d ON t.department_id = d.id
                            INNER JOIN users u ON t.user_id = u.id" . $whereClause . 
                            " ORDER BY t.teacher_code ASC LIMIT :limit OFFSET :offset";
                    ...
                    if (count($teachers) === 0) {
                        ...
                        foreach ($teachers as $t):
                            ...
                            $profilePic = !empty($t['profile_picture']) ? 'uploads/profiles/' . htmlspecialchars($t['profile_picture']) : null;
                            ?>
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <?php if ($profilePic): ?>
                                            <img src="<?= $profilePic ?>" alt="Profile" class="rounded-circle me-3"
                                        ...
                                        <?php else: ?>
                                            ...
                                            ?>
                                            <div class="avatar-circle <?= $randomColor ?> me-3"
                                                style="width:30px;height:30px;font-size:0.8rem;">
                                                <?= htmlspecialchars($avatarInitial) ?></div>
                                        <?php endif; ?>
                                        <div>
                                            <span class="fw-medium d-block"><?= $fullName ?></span>
                                            <?php if (!empty($t['line_id'])): ?>
                                                <small class="text-success"><i class="fab fa-line"></i>
                                                    <?= htmlspecialchars($t['line_id']) ?></small>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </td>
                                <td><span
                                        class="badge bg-light text-dark border"><?= htmlspecialchars($t['dept_name']) ?></span>
                                </td>
                                <td><?= htmlspecialchars($t['phone'] ?: '-') ?></td>
                                <td><?= htmlspecialchars($t['line_id'] ?: '-') ?></td>
                                <td class="text-center">
                                    <button class="btn btn-sm btn-light text-primary me-1" title="แก้ไข" data-bs-toggle="modal"
                                        data-bs-target="#editTeacherModal" onclick='openEditModal(<?= json_encode($t) ?>)'><i
                                            class="fas fa-edit"></i></button>
                                    <form action="api/teacher_actions.php" method="POST" class="d-inline"
                                        onsubmit="return confirm('ยืนยันการลบข้อมูลบุคลากรนี้ ข้อมูลผู้ใช้จะถูกลบไปด้วย?');">
    ...
    <div class="card-footer bg-white border-top py-3 d-flex justify-content-between align-items-center">
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination pagination-sm mb-0">
                ...
                <!-- Page Numbers -->
                <?php 
                // Determine start and end pages to show (e.g. max 5 buttons)
                $maxPagesToShow = 5;
                ...
                ?>
                    <li class="page-item <?= ($page == $i) ? 'active' : '' ?>"
                        <?= ($page == $i) ? 'style="--bs-pagination-active-bg: var(--accent-color); --bs-pagination-active-border-color: var(--accent-color);"' : '' ?>>
        ...
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body p-4">
                <form action="api/teacher_actions.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="teacher_code" class="form-control bg-light" placeholder="เช่น T1004"
                            required>
                    </div>
                    <div class="mb-3">
            ...
            <div class="modal-footer border-top-0 bg-light">
                ...
                <button type="submit" class="btn btn-primary"
                    style="background-color: var(--accent-color); border: none;">บันทึกข้อมูล</button>
            </div>
            </form>
        </div>
    </div>
        ...
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body p-4">
                <form action="api/teacher_actions.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-medium text-muted small">รหัสประจำตัวครู (Username)</label>
                        <input type="text" name="teacher_code" id="edit_teacher_code" class="form-control bg-light"
                            readonly>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-medium text-muted small">ชื่อ *</label>
                            <input type="text" name="first_name" id="edit_first_name" class="form-control bg-light"
                                required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-medium text-muted small">นามสกุล *</label>
                            <input type="text" name="last_name" id="edit_last_name" class="form-control bg-light"
                                required>
                        </div>
                    </div>
                    <div class="mb-3">
            ...
            <div class="modal-footer border-top-0 bg-light">
                <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">ยกเลิก</button>
                <button type="submit" class="btn btn-primary"
                    style="background-color: var(--accent-color); border: none;">อัปเดตข้อมูล</button>
            </div>
            </form>
        </div>
    </div>
